...
import WorkspaceInvitation from "./invitationModel";
...
export const inviteTeamMembers = async (
  ...
  try {
    ...
    try {
      ...
      const currentCount = await WorkspaceInvitation.countDocuments({
      ...
      const createdMemberships = await Promise.all(
        emails.map(async (email) => {
          ...
          const [membership] = await WorkspaceInvitation.create(
            [
              {
                workspace_id: workspace._id,
                email: email.email,
